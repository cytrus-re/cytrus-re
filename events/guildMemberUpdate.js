...
New Avatar URL: ${userNew.user.avatarURL}`)
    ...
    modLogChannel.send(embed).catch();
    modLogChannel.send(new Discord.MessageAttachment(user.user.avatarURL, "old.png")).catch();
    modLogChannel.send(new Discord.MessageAttachment(userNew.user.avatarURL, "new.png")).catch();
  }
};
