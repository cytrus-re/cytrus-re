...
exports.run = async (client, message, args, level) => { 
  try {
    ...
    let url = `https://djsdocs.sorta.moe/v2/embed?src=stable&q=${encodeURIComponent(args.join(" "))}`;
    fetch(url)
    .then(res => res.json())
    .then(embed => {
      if(embed && !embed.error) {
        ...
        message.channel.send(`I couldn't find anything in the Discord.JS docs for "${args.join(" ")}"...`);
      }
    })
  } catch (err) {
