exports.run = async (client, message, args) => {
  try {
    ...
    switch (args[0]) {
      ...
      case "remove":
        if (!args[1])
          return message.channel.send("You need to give me the note ID!");
        if (client.notes.has(args[1])) {
          if (
            client.notes.get(args[1]).author !== message.author.id &&
            message.author.permLevel < 6
          )
            message.channel.send("You don't own this note!");
          else {
      ...
      case "view":
        if (!args[1])
          return message.channel.send("You need to give me the note ID!");
        if (client.notes.has(args[1])) {
          if (client.notes.get(args[1]).author !== message.author.id)
            message.reply("You don't own this note!");
          else
            message.channel.send(
              "•" +
                "*" +
                client.notes.get(args[1]).id +
                "*\n" +
      ...
      case "edit":
        ...
        if (client.notes.has(args[1])) {
          if (client.notes.get(args[1]).author !== message.author.id)
            ...
            await client.notes.set(args[1], {
              txt: args.slice(2).join(" "),
              id: args[1],
              author: message.author.id,
            });
