exports.run = async (client, message, args, level) => {
  try {
    if (!args[0])
      return message.channel.send("You need to give me a message ID!");

    let msg = message.channel.messages.get(args[0]);
    if (!msg)
      return message.channel.send(
        "That message does not exist! Make sure to use this command in the same channel as the message."
      );
