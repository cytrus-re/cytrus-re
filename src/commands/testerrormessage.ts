exports.run = async (client, message, args) => {
  try {
    ...
    if (args[0] == "wiiError") message.channel.send(client.errors.wiiError);
    ...
    if (args[0] == "noResults") message.channel.send(client.errors.noResults);
    if (args[0] == "noArticleDescription")
      message.channel.send(client.errors.noArticleDescription);
  } catch (err) {
    message.channel
      .send("Okay, something actually went wrong with this command.\n" + err)
      .catch();
  }
};
