exports.run = async (client, message, args, level) => {
  try {
    ...
    if (wUser) {
      ...
      if (member) {
        if (!client.warns.get(message.guild.id))
          client.warns.set(message.guild.id, {});
        if (!client.warns.get(message.guild.id)[member.id])
        ...
        if (
          modLogChannel &&
          message.guild.channels.find((c) => c.name === settings.modLogChannel)
        ) {
          ...
          member
            ...
            .catch((err) => {
              message.channel.send(
                `I was unable to ban ${wUser.tag} for exceeding ${client.config.maxWarns} warns!`
              );
            });
        }
      } else {
