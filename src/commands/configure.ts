...
exports.run = async (client, message, args) => {
  try {
    ...
    if (args[0] === "edit") {
      ...
      if (!defaults[args[1]])
        return message.reply(args[1] + " does not exist in the settings!");
      let joinedValue = args.join(" ");
      if (joinedValue.length < 1)
        return message.reply("Please specify a new value.");
      if (joinedValue === settings[args[1].slice(2)])
        return message.reply("This setting already has that value!");

      if (!client.settings.has(message.guild.id))
        client.settings.set(message.guild.id, {});

      if (args[1] == "modLogChannel" || args[1] == "welcomeChannel")
        client.settings.set(
          message.guild.id,
          args.slice(2).join(" ").replace("#", "").trim(),
          args[1]
        );
      else
      ...
      message.reply(
        `${args[1]} successfully edited to ${args.slice(2).join(" ")}!`
      );
    } else if (args[0] === "del" || args[0] === "reset") {
      ...
      const isDefault = !overrides[args[1]]
        ...
        : "";
      message.reply(
        `The value of ${args[1]} is currently ${settings[args[1]]}${isDefault}.`
      );
    } else {
      let embed = new Discord.MessageEmbed()
        .setTitle("Server Settings")
        .setColor("0x1167b1");
      Object.keys(client.getSettings(message.guild.id)).forEach(
        (setting) => (embed = embed.addField(setting, settings[setting]))
      );
