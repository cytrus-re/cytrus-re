exports.run = async (client, message, args) => {
    ...
    if (
      ...
      return message.channel.send({
        embed: {
          ...
          description:
            "RGB color codes include **3** numbers from **0** to **255**, separated by commas and spaces",
          footer: { text: 'Example: "114, 137, 218"' },
        },
      });
    }
  ...
  function testhexvalidity(hex) {
    //tests if a hex value is valid or not
    ...
    if (hex.length != 6) {
      return message.channel.send({
        embed: {
          ...
          title: "Incorrect format!",
          description:
            "Hex colour codes include **6** of the following characters:\n1, 2, 3, 4, 5, 6, 7, 8, 9, 0, a, b, c, d, e and f",
          footer: { text: 'Example: "#7289da"' },
        },
      });
    }
  ...
  try {
    switch (args[0]) {
      case "rgbtohex":
        ...
        message.channel.send({
          embed: {
            color: hexOut,
            title: "RGB to hex",
            description: `rgb(${parseInt(args[1])}, ${parseInt(
              args[2]
            )}, ${parseInt(args[3])}) converts to \n hex #${hexOut}`,
          },
        });
        break;
      ...
      case "hextorgb":
        if (!args[1])
          return message.channel.send("You have to give me a hex code!");
        ...
        message.channel.send({
          embed: {
            color: args[1],
            title: "Hex to RGB",
            description: `${args[1]} converts to ${rgbOut}`,
          },
        });
        break;
      ...
      case "random":
        ...
        message.channel.send({
          embed: {
            color: rndHex,
            title: "Random Colour",
            description: `Your random colour is ${rndRGB}, or hex #${rndHex}`,
          },
        });
        break;
      ...
      case "viewhex":
        if (!args[1])
          return message.channel.send(
            "You have to give me a hex code!\nSee `help colourutil` for more information."
          );
        if (testhexvalidity(args[1])) {
          message.channel.send({
            embed: { color: args[1], title: `#${args[1]}` },
          });
        } else return;
