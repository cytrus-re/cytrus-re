    ...
    if (args[0] && ms(args[0])) {
      if (user) {
        ...
        if (member) {
          member
            .addRole(
              message.guild.roles.find((r) => r.name == settings.muteRole)
            )
            .then(async () => {
      ...
      message.reply(
        "You didn't specify the amount of time to mute the member or the time is invalid!"
      );
  } catch (err) {
