...
module.exports = (client, oldemoji, newemoji) => {
  ...
  if (settings.logEmojiUpdates == "true") {
    // If the "log emoji update" setting is set to true, then we should do this.
    let modLogChannel = settings.modLogChannel; // Set the logging channel

    if (
      modLogChannel &&
      newemoji.guild.channels.find((c) => c.name === settings.modLogChannel)
    ) {
      ...
      newemoji.guild.channels
        .find((c) => c.name === settings.modLogChannel)
        .send(embed); // And then we send all that to the logging channel.
    }
  }
};
