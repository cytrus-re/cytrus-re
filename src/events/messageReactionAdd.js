const Star = class {
  ...
  run(reaction, user) {
    ...
    if (
      message.guild.channels.find((c) => c.name == settings.starboardChannel) ==
      null
    )
      return;
    if (message.author.bot)
      return message.channel.send("You can't star bot messages!");
    if (user.id == message.author.id)
      return message.channel.send("You can't star your own messages!");
